...
async def test_getpost(client):
    ...
    assert post.file == {'width': 767, 'height': 1000, 'ext': 'png', 'size': 489122, 'md5': '539fd6c8c9af7b79693783b995ddf640', 'url': 'https://static1.e621.net/data/53/9f/539fd6c8c9af7b79693783b995ddf640.png'}
    assert post.preview == {'width': 115, 'height': 150, 'url': 'https://static1.e621.net/data/preview/53/9f/539fd6c8c9af7b79693783b995ddf640.jpg'}
    assert post.sample == {'has': False, 'height': 1000, 'width': 767, 'url': 'https://static1.e621.net/data/53/9f/539fd6c8c9af7b79693783b995ddf640.png'}
    assert post.score == {'up': 5, 'down': 0, 'total': 125}
    assert post.tags == {'general': ['5_fingers', 'anthro', 'bed', 'bedding', 'blanket', 'clothed', 'clothing', 'door', 'duo', 'fingers', 'fur', 'furniture', 'lying', 'male', 'male/male', 'on_side', 'pillow', 'sleeping', 'spooning'], 'species': ['bird_dog', 'canid', 'canine', 'canis', 'domestic_dog', 'golden_retriever', 'hunting_dog', 'mammal', 'retriever', 'wolf'], 'character': ['daniel_segja', 'joel_mustard'], 'copyright': ['patreon'], 'artist': ['zeta-haru'], 'invalid': [], 'lore': [], 'meta': ['comic']}
    assert post.locked_tags == []
    assert post.change_seq == 23384218
    assert post.flags == {'pending': False, 'flagged': False, 'note_locked': False, 'status_locked': False, 'rating_locked': False, 'deleted': False}
    assert post.rating == 's'
    assert post.fav_count == 306
    assert post.sources == ['https://www.furaffinity.net/view/25500100/', 'https://furaffinity.net/user/zeta-haru']
    assert post.pools == [6527]
    assert post.relationships == {'parent_id': None, 'has_children': False, 'has_active_children': False, 'children': []}
    assert post.approver_id == 38571
    assert post.uploader_id == 269143
    assert post.description == ''
    assert post.comment_count == 42
    assert post.is_favorited == False
...
async def test_post_search(client):
    assert await client.posts("m/m") != None
    assert await client.posts(["m/m", "rating:s"]) != None
    assert len(await client.posts("m/m", limit=1)) == 1
    assert await client.posts("m/m", page=1) != None
...
async def test_flags(client):
    ...
    assert flag.is_resolved == False
    assert flag.updated_at == '2020-04-19T02:50:38.030-04:00'
    assert flag.is_deletion == False
...
async def test_pools(client):
    ...
    assert pool.is_active == False
    assert pool.category == 'series'
    assert pool.is_deleted == False
